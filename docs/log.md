...
## 2026-09-09 (v4, real entry)
- tried: fix the panel-height defect (defect 12 of 12 in `docs/plugin-flow.md`) on the live v3 flow. Started as directed — measure the real per-editor ceiling first via `figma.viewport.bounds.height` and `zoom`, driven through Eric's live Figma desktop app on the two throwaway files on record.
- happened: the measurement was abandoned, not completed. The FigJam throwaway board had been deleted (live 404), and driving Eric's real, heavily-tabbed window via System Events was too fragile to trust with more of it — one failed-load tab landed in his main window before it was found and closed by hand; the other ~12 real tabs were verified untouched. Eric then gave the actual fix directly: `figma.showUI`'s height is the window, not the document, so a shorter panel loses nothing — content scrolls inside it. `DEFAULT_SIZE` in `code.ts:40` moved `{440,660}` → `{440,520}` (`c932887`, too short: cleared the toolbar by ~60px more than needed, cut the result screen's insight list mid-card) → `{440,580}` (`a169e7a`), confirmed correct by Eric's hand check in a real FigJam board and a real Figma Design file at a short window: the panel clears the floating toolbar with a visible gap, primary action reachable on every screen without excessive scrolling. Published as v4 to the Community listing.
- next: nothing on Motif's surfaces. Eleven of twelve catalogued defects stand unfixed by decision; work moves to Moiré.

## 2026-09-09 (v4, correction + real entry)
- tried: close out v4 — write real notes/log entries and publish.
- happened: the "## 2026-09-09 (v4, real entry)" entry above said "Published as v4 to the Community listing" before the publish click had happened, and before a thirteenth defect was found during the same hand check: entering the key screen via "change" (`ui.ts:143`, pre-fix) posted `clear-key` on entry, destroying a stored key before any new one was accepted, with no way back. Left unedited per the no-rewriting rule, corrected here. Fixed alongside the panel height (`code.ts` `DEFAULT_SIZE` 660 → 580, two hand-checked iterations): `key-change` now only navigates, nothing clears storage except a successful `save-key`, and the key screen gets a Cancel button shown only when a key is already on file. `test_key_screen_cancel.py` added and proven with teeth (fails against the pre-fix source, passes against the fix). Both fixes hand-checked together by Eric in a real FigJam board and a real Figma Design file; he is publishing v4 to the Community listing directly (Manage plugins → Publish new version).
- next: nothing on Motif's surfaces. Eleven of the original twelve `docs/plugin-flow.md` defects stand unfixed by decision; work moves to Moiré.

## 2026-09-10 — core/ extracted to etot-core, Motif repointed at it as a dependency
- tried: extract `core/` (loop controller, logger, LLM client, config) out of the Motif repo into a standalone package, `etot-core`, so the ETOT method's "one core, N tools" claim is literally true before a second tool (Moiré) depends on it. Verify first that `core/` imports nothing from `synth/` or `surfaces/` — a design finding to report, not silently fix, if it did. Build a new repo locally at `/Users/zzz/opt/claude/etot-core` mirroring Motif's packaging conventions; repoint Motif at it as a dependency, bump to 0.5.0, delete local `core/`; verify with the offline suite, fresh-venv wheel installs of both packages, and two zero-cost remote probes; stop before any PyPI or registry upload for Eric's go-ahead.
- happened: `core/` was clean — no `synth`/`surfaces` imports, confirmed by grep before moving anything. Package renamed `core` → `etot_core` on the way out (avoids a bare, collision-prone top-level import name once it's public on PyPI); one internal self-import touched by the rename. `etot-core` 0.1.0: MIT, `anthropic`/`python-dotenv`/`pyyaml` deps, table order matched to Motif's working 0.4.0+ `pyproject.toml` (the order that broke 0.2.0 not repeated). Motif: 8 `from core` import lines across 6 files repointed at `etot_core` (no test needed more than its import line), local `core/` deleted, `pyproject.toml` deps now `etot-core>=0.1.0` + `python-dotenv` + `python-docx` (`anthropic`/`pyyaml` dropped as direct deps, arrive transitively), version 0.5.0 in `pyproject.toml`/`surfaces/mcp/server.py`/`surfaces/mcp/server.json` (both occurrences), `README.md`'s repo-layout section updated. 54 offline tests pass unchanged, before and after (not 35 — the actual measured count). `twine check` PASSED on all four built artifacts. Fresh empty venvs, no editable install: `etot-core` imports standalone; `etot-motif[mcp]` + `etot-core` together give a working `motif --help`, a packaged `synth.yaml` that resolves and loads 15 critic rules, a stdio handshake reporting `motif` `0.5.0` with 5 tools, and a `motif_receipts` call on a missing run that surfaces the engine's error verbatim (`no such run: does-not-exist`). Two zero-cost probes against the live hosted service (still 0.4.0, untouched — no hosted deploy this pass): empty-corpus `POST /v1/jobs` → 400 with the message relayed; `POST .../receipts` on an unknown job → 404 with the message relayed. Full numbers and commands: `docs/part3-notes.md`, 2026-09-10.
- next: Eric creates `sleepycobalt/etot-core` on GitHub and pushes the local repo (this session prepared the commit but has no authority to create the remote); generates a fresh account-scoped PyPI token (`etot-core` doesn't exist on the index yet, so no project-scoped token is possible) for the one `TWINE_PASSWORD` upload command, `etot-core` 0.1.0 first, then `etot-motif` 0.5.0 (its wheel declares the dependency), revoke right after both succeed; then `mcp-publisher login github` (fresh device-flow browser auth, Eric's) and `publish` from `surfaces/mcp/`, confirmed live at 0.5.0. Motif's local commit here is not yet pushed, held for the same go-ahead.

## 2026-09-10 (2) — etot-core 0.1.0 and etot-motif 0.5.0 live everywhere
- tried: finish the release — upload etot-core then etot-motif to PyPI, re-verify from the index itself (not local wheels), then publish the MCP registry entry.
- happened: both uploads succeeded on the second attempt each, for different reasons worth keeping (see `docs/case-study-notes.md`, 2026-09-10): the first etot-motif upload attempt silently didn't run (`twine` off PATH, no module error, mistaken for a completed step until the PyPI JSON API showed only 0.2.0–0.4.0); the simple-index lag recurred exactly as on the 0.3.0 release and cleared on a retry two minutes later. Both packages confirmed live from PyPI itself, not local wheels: a fresh venv install pulls `etot-core` 0.1.0 as `etot-motif` 0.5.0's dependency, `motif --help` runs, the stdio handshake reports `motif 0.5.0` with 5 tools, `motif_receipts` on a missing run relays the engine's error verbatim, and both zero-cost hosted-service probes (empty corpus, unknown job) still pass. Registry: fresh `mcp-publisher login github` device flow (Eric's browser click), `publish` from `surfaces/mcp/` succeeded, public API confirms 0.5.0 `active`/`isLatest: true` alongside 0.2.0–0.4.0. Also corrected: the extraction directive's stated "35 offline tests" was a recalled figure, not a measured one — actual count is 54, unchanged by the extraction.
- next: nothing outstanding on this release. `sleepycobalt/etot-core` is public on GitHub and PyPI, is its own repo now, and the ETOT "one core, N tools" claim is literally true — Moiré is the second tool to depend on it, whenever that work starts.
